...
Ac = [0.24 0 0.1787 0;-0.3722 1 0.2703 0;-0.9901 0 -0.1389 0;-48.9354 64.1 2.3992 1];
...
Y_p_ = [0.349;100;100]; Y_n_ = [-0.349;-100;-100];       %MIMO时这两个变量为向量
%将约束扩展到M（P）个时域
...
delta_u_ini = 0.01*ones(nu*M,1);
y_ini = 0.01*ones(mc,1);
lambda_ini = 0.01*ones(mc,1);

%确定参考轨迹
rr1 = 400*ones(1,N_sim);
rr2 = zeros(1,N_sim);
rr3 = zeros(1,N_sim);
rr = []; 
for i=1:P
...
for kk = 1:N_sim;
    ...
    %tic
    [delta_u,delta_u_M_in,u_k,y_k,x_k,delta_u_ini,y_ini,lambda_ini] = mpc_v3_model3(delta_u,delta_u_ini,u_k,y_k,x_k,r_k,delta_u_ini,y_ini,lambda_ini);%调用MPC在线算法进行计算
...
%画图
u_draw = u_draw/3.1415*180;
figure;
subplot(4,1,1);plot(rr(1,:),'-.');hold on;plot(y_draw(:,2),'LineWidth',2,'Color','r');  title('Plant Output: yp(k)'); %axis([0 N_sim -2 2]);
...
hold on; plot(20*ones(N_sim,1),'--');hold on; plot(-20*ones(N_sim,1),'--');
