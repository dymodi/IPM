...
function [xstar,zstar,zstar1,Iter] = IPM_v2_full(G,GL,c,A,b,x,y,lambda)
global global_res;
[m,n] = size(A);
maxIteration = 40;
...
if(min(A*x_try-b)>0)
   ...
   zstar = 0.5*x'*G*x + c'*x;
    ...
    for k = 1:maxIteration
        ...
        F = G+At*y_lambda_A;
        F_full = [G zeros(n,m) -At;A -eye(m,m) zeros(m,m);zeros(m,n) diag(lambda) diag(y)];
         
        sig_mu = sigma*mu*ones(m,1);
        center_part = rp_y + sig_mu./lambda;
        equation_b = -rd+At*(inv_y_lambda.*center_part);
        equation_b_full = [-rd;-rp;-diag(y)*diag(lambda)*ones(m,1)+sig_mu];
...
%         delta_full = linsolve(F_full,equation_b_full);
%         delta_x = delta_full(1:n,1);
%         delta_y = delta_full(n+1:n+m,1);
%         delta_lambda = delta_full(n+m+1:n+2*m,1);
...
% 
        ...
        [flag,fx_old,x_old] = TC_Himmelblau(G,c,A,b,fx_old,x_old,x,0.001);
        if(flag==1)
            break;
        end
