%这是用来测时间的一段小程序，初值给的是第二周期的值，和VS中测时间的条件一样
%testTime_of_MPC
...
delta_u = [0.1;0.1];             %用以给优化过程的x赋第一周期的初值；
...
tic;
for kk = 1:300;
    r_k = rr(:,1);        %对k时刻的参考轨迹进行更新
    [delta_u,u_k,y_k,x_k] = mpcTime(delta_u,u_k,y_k,x_k,r_k);%调用MPC在线算法进行计算
%     
